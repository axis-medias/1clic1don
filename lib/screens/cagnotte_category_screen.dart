...
// Catégories communes (identique au site web et à cagnotte.dart)
...
class CagnotteCategoryScreen extends StatefulWidget {
  final int categoryId;
  final String categoryName;

  const CagnotteCategoryScreen({
    super.key,
  ...
  @override
  State<CagnotteCategoryScreen> createState() => _CagnotteCategoryScreenState();
}

class _CagnotteCategoryScreenState extends State<CagnotteCategoryScreen> {
  ...
  Future<void> _loadCagnottes() async {
    ...
    try {
      ...
      final url = Uri.parse(
        'https://www.1clic1don.fr/app/cagnottes_par_categorie.php'
            '?id=${widget.categoryId}&lang=$lang&page=$_currentPage&limit=$_limit',
      );
      ...
      debugPrint('CagnotteCategoryScreen: error: $e');
      if (mounted) {
        setState(() {
          _errorMessage = 'error_technical_difficulties'.tr();
          _isLoading = false;
          _isError = true;
        });
      }
  ...
  Widget _buildContent() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          // Mobile : ListView en colonne simple
          if (isMobile)
            ListView.separated(
              shrinkWrap: true,
              ...
              itemBuilder: (context, index) {
                return _buildCagnotteMobileCard(_cagnottes[index]);
              },
            )
